...
# usingExamples <- 1:10000
# testData <- data[usingExamples, usingFeatures]
# testY <- y[usingExamples,]

# dataScale <- scale(data[,usingFeatures])

nn <- nnet(x=data[,usingFeatures], y=y, size=300, MaxNWts=4000, decay=1)
...
# dataScale_test <- scale(data_test[,usingFeatures])

predict <- predict(object=nn, newdata=data_test[,usingFeatures])
