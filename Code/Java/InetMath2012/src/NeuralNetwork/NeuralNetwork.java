...
public class NeuralNetwork {
    ...
    public static final int ALLOW_EXAMPLES_NUMBER = 5000000;
    public static final int TEST_EXAMPLES_NUMBER = 738977;
    ...
    public static void main(String[] args) {
        ...
        NeuralDataSet trainingSet = getDataSet(fileSession, fileUser, ALLOW_EXAMPLES_NUMBER);

        BasicNetwork network = trainNeuralNetwork(trainingSet);

        NeuralDataSet testSet = getDataSet(fileTestSession, fileTestUser, TEST_EXAMPLES_NUMBER);

        predict(testSet, network, fileResult);
    ...
    private static NeuralDataSet getDataSet(File fileSession, File fileUser, int examplesNumber) {
        ...
        try {
            ...
            double[][] OUTPUT = new double[examplesNumber][1];
            double[][] DATA = new double[examplesNumber][FEATURES_NUMBER];
            ...
            while (scannerSession.hasNextLine() && scannerUser.hasNextLine() && indexExamples < examplesNumber) {
                ...
                OUTPUT[indexExamples][0] = Double.parseDouble(dataSession[0]);

                indexFeatures = 0;
                for (int i = 1; i < dataSession.length; ++i) {
                    DATA[indexExamples][indexFeatures] = Double.parseDouble(dataSession[i]);
                    ++indexFeatures;
                }
                for (int i = 0; i < dataUser.length; ++i) {
                    DATA[indexExamples][indexFeatures] = Double.parseDouble(dataUser[i]);
                    ++indexFeatures;
                }
            ...
            trainingSet = new BasicNeuralDataSet(DATA, OUTPUT);
